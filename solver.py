...
def save_metrics_to_csv(csv_filename, metrics_data, epoch):
   
    if epoch == 1 and os.path.isfile(csv_filename):
        os.remove(csv_filename)  

    with open(csv_filename, mode='a', newline='') as file:
        writer = csv.writer(file)

        if epoch == 1 and not os.path.isfile(csv_filename):
            header = ['epoch']
        ...
        row = [epoch]
        for metric_name, metric_meter in metrics_data.items():
...
class Solver:
    ...
    def __init__(
        ...
        *,
        ...
        if misc.typename(self.optimizer) == "LBFGS":
            ...
            self.train_epoch_func = train_LBFGS_epoch_func
            if self.update_freq != 1:
                self.update_freq = 1
                logger.warning("Set 'update_freq' to to 1 when using L-BFGS optimizer.")
        else:
            self.train_epoch_func = train_epoch_func
    ...
    def eval(
        ...
        csv_filename = "eval_metrics.csv"  
        save_metrics_to_csv(csv_filename, self.eval_output_info, epoch_id)
